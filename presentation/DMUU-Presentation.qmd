...
All 150,000 matches in out game are done in different matches. This means the outcome of the matches do not influence eachother. Because of this we know each data point is independent.
...
ggplot(match_stats, aes(x = TotalGold, y = DmgDealt)) +
  geom_point() +
  geom_smooth(method = "lm") +
...
ggplot(match_stats, aes(x = MinionsKilled, y = DmgDealt)) +
  geom_point() +
  geom_smooth(method = "lm") +
...
## Damage Dealt vs Current Mastery

```{r scatterplots to check for linearity taken vs mastery points}

ggplot(match_stats, aes(x =CurrentMasteryPoints , y =  DmgDealt)) +
  geom_point() +
  geom_smooth(method = "lm") +
...
Because multiple predictors did not show a linear relationship with damage dealt, we simplified the model to keep only the visually linear predictors.
...
## Checking for no multicollinearity {.big-code}
...
## Variance Inflation Factor {.big-code}

A VIF of 1 means there is no correlation with other predictors. This only really matters if the VIF is greater than 5 as this suggests a high multicollinearity. Based on the test, both of our variables have a pretty low multicollinearity and are safe to use.
...
To check for normality of residuals we use a Q-Q plot of the residuals.
Using `which = 2` prints a Q-Q Residuals graph or Quantile-Quantile plot of residuals.
If this is straight diagonal it means the residuals are about normal.
...
The graph not being straight diagonal is okay for our data through. This is alright because of our large number of 150,000 samples and the Central Limit Theorem. The coefficient values produced by the linear regression become approximately normaly distributed because of the large dataset.
...
Because our p value is so low we reject the idea that we have homoscedasticity which is a requirement for for correct p values during multiple regression. This is okay as we can use a different test to find the p-values.
...
This is a summary of our multiple linear regression without accounting for heteroscedasticity. The correlation between damage dealt, damage taken, and total gold is statistically significant.
...
This uses the previous multiple linear regression calculations but holds all other variables constant.
...
# Reduces the amount of points used in the rendering of the graph
match_stats_smaller <- match_stats[1:15000, ]

# Puts in the dots
three_dimension_graph <- add_markers(
  p = three_dimension_graph, # Adds onto the plot I already made
  data = match_stats_smaller,
  x = ~DmgTaken,
  y = ~TotalGold,
  z = ~DmgDealt,
  marker = list(
...
This is only rendering the first 15,000 points. It still uses the regression of all 150,000.

## Multiple Linear Regression Conclusion

From this test we can confidently conclude that damage taken and total gold are hightly significant predictors of the damage a player has dealt. Players who take more damage and accumulate more gold deal reliably higher damage. This is as expected from the core game mechanics.
...
## Two Way Anova
Another important part of the game is Runes, a series of passive abilites you get to choose before a match starts. To determine the best runes to take per lane, we run a Two Way ANOVA test. By visualizing the results as a series of boxplots we can visulize the mean average damage per rune, which shows us which over or under perform. The F values show us how important each metric is against the random noise of the data. Although it should be noted that the large amount of residuals show that damage is mostly determined by other factors in the game such as gold gained and individual differences in player skill.
...
## Team Playstyle Logistical Regression
Looking at this summary we can see the individual effects each factor has on your chance of winning. Based on these coefficents we can assign a value to each map objective and see how valuable each on is in terms of increasing your winrate. This shows that it is by far more efficent to play for any of the objectives over kills, with dragons being the most valuable. Using these coefficents we can create a simple winrate predictor by feeding a team stat line in.

## Team Playstyle Logistical Regression {.big-code}
