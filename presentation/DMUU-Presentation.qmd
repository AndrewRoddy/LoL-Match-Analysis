...
This uses the previous multiple regression claculations but holds all other variables constant.
...
## Partial Regression {.big-code}
```{r partial regression dmgdealt vs minionskilled }
avPlots(
  multiple_regression,
  "MinionsKilled",
  col = alpha("#000000", 0.04),
...
```
Compares Damage Dealt with Minions Killed.

## Partial Regression {.big-code}
Takeaway: Support players are intended to kill minions and not deal damage. This makes sense.



## Partial Regression {.big-code}
```{r partial regression dmgdealt vs towersdowned }
avPlots(
  multiple_regression,
  "TurretDmgDealt",
...
```
Note the different graph scales. Also TurretDmgDealt means Turret's Downed.

## Partial Regression {.big-code}
Takeaway : Players that destroy towers are less likely to deal a lot of damage.
...
## Chi Square Test - Build Optimizer
    We run a Chi-Square test to see if the difference between Keystones is statistically signifigant. In other words if the choice of keystone matters in determining winrate.
##
...
We also run a Fisher Test on the items to see if there is a statistically signifigant relation between items and winrate

```{r risher test}
# Is difference between them signifigant
test_table <- table(ahri_data$item, ahri_data$Win)
fisher.test(test_table) #Alternative chi^2 because of some items low sample size
