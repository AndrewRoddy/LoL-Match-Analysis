...
library(dplyr)
library(tidyr)
#Pull all data from matchstats table and find the optimum build and rune setup for each character. Where the build appears more than once.
#First Aggregate all builds that appear more than once
#Using Ahri as a proof baseline ChampID:103
library(dplyr)
#Filter to only Ahri games where the Mode was not some alternate gamemode (Nomral Summoners Rift Only)
...
unique_items <- unique(unlist(ahri_data[c('item1', 'item2', 'item3', 'item4', 'item5', 'item6')]))

ahri_item_data <- ahri_data |>
  pivot_longer(cols = c('item1', 'item2', 'item3', 'item4', 'item5', 'item6'),
                 names_to = 'slots',
                 values_to = 'item')
