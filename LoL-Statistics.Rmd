...
match_metadata <- read.csv("data/other/MatchTbl.csv")
match_stats <- match_stats |>
  left_join(match_metadata, by = c("MatchFk" = "MatchId"))
```
...
```
Values between 1 and two no multicollinearity for out test.
...
ggplot(match_stats, aes(x = DmgTaken, y = DmgDealt)) +
  ...
  labs(title = "Linearity: DmgDealt vs DmgTaken")
...
ggplot(match_stats, aes(x = DmgTaken, y = TotalGold)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(title = "Linearity: DmgDealt vs TotalGold")
...
ggplot(match_stats, aes(x = DmgTaken, y = MinionsKilled)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(title = "Linearity: DmgDealt vs MinionsKilled")
...
ggplot(match_stats, aes(x = DmgTaken, y = TurretDmgDealt)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(title = "Linearity: DmgDealt vs TurretDmgDealt")

```
This one isn't linear either
...
ggplot(match_stats, aes(x = DmgTaken, y = CurrentMasteryPoints)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(title = "Linearity: DmgDealt vs CurrentMasteryPoints")

```
This one also ins't linear either

After checking we only have two variables that have linear relationships with DmgTaken.

So we will do the multiple regression again with just those two variables.
```{r multiple regression run}
...
This is our p values before accounting for this.
```{r prints the summary}
# Print off if this matters or not
summary(multiple_regression)
```

This is them while using a test that accounts for this. 
```{r coefficient test}
library(sandwich)
library(lmtest)
...
```
Luckily our p values are so small not having homoscedasticity doesn't have a large effect. This test calcuates and uses robust stardard errors which accounts for not having homoscedasticity. They ensure the standard errors are valid even when the residual variance is not constant.
...
Makes a 3d graph of multiple regression.
```{r 3d plot}
library(plotly)
